...
# Upload data
tAI = pd.read_csv("results/RtAI_cellmeans.csv", index_col=0)
tropism = pd.read_csv("data/virus_list.tsv", sep="\t" ,index_col=0)

# Create input dataframe with features
indf = pd.DataFrame(tAI.iloc[:,0:7])
# Associate each row to target values
indf.loc[:,"tropism"] = [tropism.loc[s,"tropism"] for s in tAI.Species]
...
colnames = ["roc_auc","accuracy"]; colnames.extend(list(indf.columns[0:7]))
RFmeans = pd.DataFrame(index = tropisms, columns = colnames)
RFstd = pd.DataFrame(index = tropisms, columns = colnames)
ratios = pd.DataFrame(index = tropisms, columns = list(indf.columns[0:7]))

for t in tropisms:
    X = indf.iloc[:,0:7]  # Features
    ...
    features = pd.DataFrame(index=indf.columns[0:7])
    ...
    ratios.loc[t,:] = np.log2(X.loc[y,:].mean(0)/X.loc[~y,:].mean(0))
...
#%% #Creating a bar plot of feature importances
    ...
    plt.show()
    fig.savefig(str("plots/RandomForest_cells/features_proteins_%s.pdf" % t), bbox_inches='tight')
#%% #ROC curve
    fig = plt.figure()
    ...
    # show the plot
    plt.show()
    fig.savefig(str("plots/RandomForest_cells/roc_proteins_%s.pdf" % t), bbox_inches='tight')
...
#%% #Precision-Recall Curve
    ...
    # show the plot
    plt.show()
    fig.savefig(str("plots/RandomForest_cells/prec-recall_proteins_%s.pdf" % t), bbox_inches='tight')

#%% Save
RFmeans.to_csv("results/mean_randomforest_cells_proteins_bytropism.csv")
RFstd.to_csv("results/std_randomforest_cells_proteins_bytropism.csv")
ratios.to_csv("results/ratios_randomforest_cells_proteins_bytropism.csv")