...
for t in tropisms:
    ...
    for n in range(100):
        ...
        for i, (train, test) in enumerate(cv.split(Xtemp, ytemp)):
            ...
            interp_tpr = interp(linear_xaxis, fpr, tpr) 
            interp_tpr[0] = 0.0
            interp_precision = interp(linear_xaxis, precision, recall) 
...
#%% #Creating a bar plot of feature importances
    ...
    plt.show()
    fig.savefig(str("plots/RandomForest/features_proteins_%s.pdf" % t), bbox_inches='tight')
#%% #ROC curve
    fig = plt.figure()
    ...
    # show the plot
    plt.show()
    fig.savefig(str("plots/RandomForest/roc_proteins_%s.pdf" % t), bbox_inches='tight')
...
#%% #Precision-Recall Curve
    ...
    # show the plot
    plt.show()
    fig.savefig(str("plots/RandomForest/prec-recall_proteins_%s.pdf" % t), bbox_inches='tight')

#%% Save
RFmeans.to_csv("results/mean_randomforest_proteins_bytropism.csv")
RFstd.to_csv("results/std_randomforest_proteins_bytropism.csv")
